...
/*
	이러한 것들 외에도 트리구조는 상당히 많다.
	1. 우리가 만든 이진트리
	2. 완전 이진트리
	3. 완전한 이진 트리(우선순위구현에 사용되는 힙)
	4. 완벽한 이진 트리(모든 부모가 자식두명을 가지는 트리로 균형잡힌 이진 검색 작업에 이상적)
	5. 균형 이진 트리(왼쪽 오른쪽 자식간의 높이차가 최대 1인 이진 트리)
	6. 이진 검색 트리
	7. AVL트리
	8. 레드-블랙 트리
	9. B-트리(부모가 2개 이상의 자식을 가질 수 있는 트리 - 데이터베이스에 사용)
	10. B+트리(모든 값이 자식 노드에 저장되고 내부 노드에는 키만 저장되는 B-트리의 변형)
	11. 접두사 트리
	12. 접미사 트리
	13. 더미(heap)
	14. 세그먼트 트리
